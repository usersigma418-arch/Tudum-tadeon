...
def create_lyric_video(cover_bytes, audio_file, lyrics_text, duration, output_file):
    ...
    try:
        ...
        # Scale to video size (1080x1920 for mobile, or 1280x720 for desktop)
        video_width, video_height = 1080, 1920
        cover_img = cover_img.resize((video_width, video_height), Image.Resampling.LANCZOS)
        
        # Apply strong blur
        blurred = cover_img.filter(ImageFilter.GaussianBlur(radius=30))
        ...
        if lyrics_text:
            ...
            # Wrap and center lyrics
            wrapped_lyrics = textwrap.fill(lyrics_text[:500], width=20)
            
            # Add semi-transparent background for readability
            y_text = video_height // 3
            lines = wrapped_lyrics.split('\n')
            
            for i, line in enumerate(lines[:12]):  # Max 12 lines
                y_pos = y_text + (i * 60)
                if y_pos > video_height - 200:
                    break
                
                # Calculate text bbox for background
                bbox = draw.textbbox((0, 0), line, font=font)
                text_width = bbox[2] - bbox[0]
                
                # Draw semi-transparent background
                bg_x1 = (video_width - text_width) // 2 - 20
                bg_y1 = y_pos - 10
                bg_x2 = (video_width + text_width) // 2 + 20
                bg_y2 = y_pos + 50
        ...
        # fps = 1 means 1 frame per second (static frame for entire duration)
        cmd = [
            "ffmpeg", "-y",
            "-loop", "1",
            "-i", frame_path,
...
async def send_song(upd, ctx, msg, sid, sinfo):
    ...
    if cover_bytes:
        await msg.edit_text(card + "\n\n🎬 Creating video with lyrics...", parse_mode="Markdown")
        loop = asyncio.get_event_loop()
        video_created = await loop.run_in_executor(
            executor,
            create_lyric_video,
            cover_bytes,
            temp_audio_path,

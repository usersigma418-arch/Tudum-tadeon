...
def create_lyric_video(cover_bytes, audio_file, lyrics_text, duration, output_file):
    """
    Create video FAST - sacrifice quality for speed
    Max speed optimized for Render
    """
    try:
        logger.info(f"🎬 Starting FAST video creation (duration: {duration}s)")
        
        # Load and process cover image - ULTRA SMALL
        cover_img = Image.open(io.BytesIO(cover_bytes)).convert("RGB")
        
        # AGGRESSIVE: Ultra small resolution for MAXIMUM speed
        video_width, video_height = 640, 360  # Very small = very fast!
        cover_img = cover_img.resize((video_width, video_height), Image.Resampling.BILINEAR)  # Fast resize
        
        # Minimal blur (radius 10 = super fast)
        logger.info("🎨 Applying minimal blur...")
        blurred = cover_img.filter(ImageFilter.GaussianBlur(radius=10))
        ...
        if lyrics_text and len(lyrics_text.strip()) > 0:
            ...
            # Super short lyrics for speed
            wrapped_lyrics = textwrap.fill(lyrics_text[:300], width=15)
            y_text = video_height // 4
            lines = wrapped_lyrics.split('\n')
            
            for i, line in enumerate(lines[:6]):  # Max 6 lines only
                y_pos = y_text + (i * 35)
                if y_pos > video_height - 80:
                    break
                ...
                try:
                    ...
                    overlay_draw.rectangle(
                        [(video_width - text_width) // 2 - 10, y_pos - 5,
                         (video_width + text_width) // 2 + 10, y_pos + 30],
                        fill=(0, 0, 0, 120)
                    )
        ...
        # Save frame - LOW QUALITY
        frame_path = f"{tempfile.gettempdir()}/frame_{int(time.time())}.jpg"  # JPG not PNG for speed
        logger.info(f"💾 Saving frame...")
        frame.save(frame_path, quality=60)  # Very low quality = tiny file = fast
        
        # ULTRA FAST FFmpeg settings
        logger.info("⚙️ Running FFmpeg (MAXIMUM SPEED)...")
        cmd = [
            "ffmpeg", "-y",
            "-loop", "1",
            "-i", frame_path,
            "-i", audio_file,
            "-c:v", "libx264",
            "-preset", "superfast",        # ⚡ MAXIMUM SPEED
            "-crf", "50",                  # 🔴 VERY LOW quality (max 51)
            "-c:a", "libmp3lame",          # Faster than AAC
            "-b:a", "64k",                 # 🔴 SUPER LOW audio bitrate
            "-q:a", "9",                   # Low quality audio
            "-shortest",
            "-pix_fmt", "yuv420p",
            "-movflags", "+faststart",
            "-loglevel", "quiet",          # No logging = faster
            output_file
        ]
        
        logger.info(f"Running FFmpeg TURBO MODE...")
        
        try:
            result = subprocess.run(
                cmd, 
                capture_output=True, 
                timeout=60,  # 🔴 REDUCED timeout to 60s
                text=True
            )
            ...
            logger.info(f"✅ Video created SUPER FAST!")
        except subprocess.TimeoutExpired:
            logger.error("❌ FFmpeg timeout even at max speed")
            return False
        except Exception as e:
...
async def send_song(upd, ctx, msg, sid, sinfo):
    ...
    # Create video with lyrics - MAXIMUM SPEED MODE
    temp_video_path = f"{tempfile.gettempdir()}/video_{int(time.time())}.mp4"
    
    if cover_bytes:
        await msg.edit_text(card + "\n\n🎬 Creating video (TURBO MODE)...", parse_mode="Markdown")
        ...
        try:
            video_created = await asyncio.wait_for(
                ...
                timeout=90  # 90 seconds max (very aggressive)
            )
            
            if not video_created:
                logger.warning("Video creation failed, falling back to audio")
                await msg.edit_text(card + "\n\n⚠️ Video failed, sending audio...", parse_mode="Markdown")
                use_video = False
            else:
                use_video = True
                logger.info("✅ Video created!")
        except asyncio.TimeoutError:
            logger.error("❌ Video timeout")
            await msg.edit_text(card + "\n\n⏱️ Video timeout, sending audio...", parse_mode="Markdown")
            use_video = False
        except Exception as e:
            logger.error(f"❌ Video error: {e}")
            await msg.edit_text(card + "\n\n❌ Video error, sending audio...", parse_mode="Markdown")
            use_video = False
    else:
        use_video = False
    ...
    # Add lyrics to caption if available
    if lyrics_text:
        # Limit lyrics to fit Telegram caption (4096 chars max)
        lyrics_preview = lyrics_text[:1500]  # First 1500 chars
        caption += f"\n📜 **LYRICS:**\n{lyrics_preview}...\n" if len(lyrics_text) > 1500 else f"\n📜 **LYRICS:**\n{lyrics_preview}\n"
    
    caption += f"\n⚡ JioSaavn Bot"

    await msg.edit_text(card + "\n\n📤 Uploading...", parse_mode="Markdown")
    try:
        if use_video and os.path.exists(temp_video_path):
            # Send video
        ...
        await msg.edit_text(f"❌ Upload fail: `{str(e)[:80]}`", parse_mode="Markdown")
    finally:
        # Cleanup temp files
        try:

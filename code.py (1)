import os, re, io, json, time, logging, asyncio, html as html_mod
...
def webhook():
    if not bot_app: return "Init pending", 503
    try:
        upd = Update.de_json(request.get_json(force=True), bot_app.bot)
        asyncio.run_coroutine_threadsafe(bot_app.process_update(upd), bot_app.loop)
        return "OK", 200
    except Exception as e:
...
async def init():
    ...
    logger.info(f"✅ Webhook: {WEBHOOK_URL}/{BOT_TOKEN[:10]}...")
